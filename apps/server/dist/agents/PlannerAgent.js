...
class PlannerAgent extends BaseAgent_js_1.BaseAgent {
    async planSprint(userPrompt) {
        this.log(`\x1b[34m[SPRINT GOAL RECEIVED]\x1b[0m "${userPrompt}"`, 'info');
        this.streamTerminal(`\x1b[1;36m========================================================\x1b[0m\r\n` +
        ...
        const generatedTasks = this.generateTasksForPrompt(userPrompt);
        ...
        this.streamTerminal(`\x1b[32m✔ Sprint planning finalized.\x1b[0m Notifying Coder Jim Halpert.\r\n`);
        // Notify Coder agent
        const firstTask = generatedTasks[0];
        this.sendMessage('coder', `Sprint Ready: ${firstTask.title}`, `Please claim task "${firstTask.title}" (${firstTask.id}) and begin implementation in ./workspace.`, 'TASK_ASSIGNED', firstTask.id);
        await this.sleep(700);
        this.setAction('Returning to desk...', 'WALKING');
        await this.returnHome();
        return generatedTasks;
    }
    generateTasksForPrompt(prompt) {
